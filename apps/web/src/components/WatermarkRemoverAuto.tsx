...
type CanvasExportApi = { convertToBlob?: (options?: { type?: string }) => Promise<Blob>; toBlob?: (callback: (blob: Blob | null) => void, type?: string) => void };
...
async function canvasToBlob(canvas: HTMLCanvasElement | OffscreenCanvas, type = "image/png"): Promise<Blob> {
  ...
  if (canvas instanceof HTMLCanvasElement && typeof api.toBlob === "function") return new Promise<Blob>((resolve, reject) => api.toBlob?.(blob => blob ? resolve(blob) : reject(new Error("Could not encode the cleaned image.")), type));
...
function reverseAlpha(ctx: CanvasRenderingContext2D, rect: Rect, alpha: Float32Array) {
  ...
  if (w <= 0 || h <= 0) return;
  const pixels = ctx.getImageData(x, y, w, h), size = Math.max(1, Math.round(Math.sqrt(alpha.length)));
  for (let row = 0; row < h; row++) for (let col = 0; col < w; col++) {
    const ax = clamp(Math.floor(col / Math.max(1, w) * size), 0, size - 1), ay = clamp(Math.floor(row / Math.max(1, h) * size), 0, size - 1);
    const raw = alpha[ay * size + ax] || 0, a = Math.min(Math.abs(raw), 0.99); if (a < 0.01) continue;
...
async function detectImageWatermark(image: HTMLImageElement | HTMLCanvasElement) {
  const first = await removeWatermarkFromImage(image, detectionOptions);
  if (first.meta?.applied && first.meta.position) return first;
  // The upstream SDK exposes a second, conservative located fallback for
  // cases where catalog/anchor detection sees the Gemini geometry but the
  // normal confidence gate rejects the restoration. This is still validated
  // by the SDK; it is not a blind crop or fixed-coordinate erase.
  return removeWatermarkFromImage(image, { ...detectionOptions, adaptiveMode: "always" });
}
...
export function WatermarkRemoverAuto() {
  ...
  const selectFile = (f: File | null) => {
    ...
    const nextKind = f.type.startsWith("video/") ? "video" : "image";
    setFile(f); setKind(nextKind); setSource(remember(URL.createObjectURL(f))); setClean(""); setMeta(null); setPosition(null); setProgress(0);
    setMessage(nextKind === "video" ? "Video loaded. The first frame will locate the fixed visible sparkle automatically." : "Image loaded. Locating the visible Gemini sparkle…");
  };

  const processImage = async () => {
    const image = imageRef.current; if (!image) return;
    setBusy(true); setMessage("Locating the visible sparkle with automatic + adaptive fallback detection…");
    try {
      const result = await detectImageWatermark(image);
      setMeta(result.meta); const rect = toRect(result.meta?.position); setPosition(rect);
      if (!result.meta?.applied || !rect) return setMessage(`No verified Gemini visible watermark was found${result.meta?.skipReason ? ` (${result.meta.skipReason})` : ""}. Nothing was blurred or guessed.`);
  ...
  const processVideo = async () => {
    if (!videoRef.current) return; setBusy(true); setProgress(0); setClean(""); setMessage("Checking the first frame with automatic + adaptive fallback detection…");
    try {
      ...
      const width = video.videoWidth, height = video.videoHeight; if (!width || !height) throw new Error("The video has no usable dimensions.");
      video.pause(); video.currentTime = 0; await new Promise<void>(resolve => { if (video.readyState >= 2) resolve(); else video.addEventListener("seeked", () => resolve(), { once: true }); });
      const engine = await createWatermarkEngine(), frame = document.createElement("canvas"); frame.width = width; frame.height = height;
      const fctx = frame.getContext("2d", { willReadFrequently: true }); if (!fctx) throw new Error("Canvas processing is unavailable in this browser."); fctx.drawImage(video, 0, 0, width, height);
      const detection = await detectImageWatermark(frame); setMeta(detection.meta); const rect = toRect(detection.meta?.position); setPosition(rect);
      if (!detection.meta?.applied || !rect) return setMessage(`No verified Gemini visible watermark was found in the first frame${detection.meta?.skipReason ? ` (${detection.meta.skipReason})` : ""}. No video frames were altered.`);
      const alpha = await engine.getAlphaMap(Math.max(8, Math.round(Math.max(rect.w, rect.h))));
      const canvas = document.createElement("canvas"); canvas.width = width; canvas.height = height; const ctx = canvas.getContext("2d", { willReadFrequently: true }); if (!ctx) throw new Error("Canvas processing is unavailable in this browser.");
      ...
      const recorder = new MediaRecorder(stream, { mimeType: mime, videoBitsPerSecond: 12_000_000 }), chunks: Blob[] = []; recorder.ondataavailable = e => { if (e.data.size) chunks.push(e.data); };
      ...
      recorder.start(250); video.currentTime = 0; await video.play(); requestAnimationFrame(draw); await new Promise<void>(resolve => video.addEventListener("ended", () => resolve(), { once: true })); recorder.stop(); await stopped; stream.getTracks().forEach(t => t.stop());
      const blob = new Blob(chunks, { type: mime }); if (blob.size < 1024) throw new Error("The cleaned video export was empty."); setClean(remember(URL.createObjectURL(blob))); setProgress(1); setMessage("Visible watermark removed from the fixed region across the video. Export is ready as WebM.");
  ...
  return <div style={{ maxWidth: 980, margin: "0 auto", borderRadius: 18, background: "#f7f7fb", border: "1px solid #e8e8ef", padding: 18, color: "#1f1f28", fontFamily: "Inter,system-ui,sans-serif" }}>
    ...
    <div onClick={() => inputRef.current?.click()} onDragOver={e => e.preventDefault()} onDrop={e => { e.preventDefault(); selectFile(e.dataTransfer.files?.[0] || null); }} style={{ border: "2px dashed #c9c9d8", borderRadius: 14, padding: "34px 24px", textAlign: "center", background: "white", cursor: "pointer" }}>
      ...
      <div style={{ fontSize: 16, fontWeight: 800 }}>Upload Gemini image or video</div><div style={{ marginTop: 7, color: "#747482", fontSize: 13 }}>Automatic visible-sparkle detection with an adaptive fallback — no manual X/Y positioning.</div>
    </div>
    {file && <>
      ...
      <div className="wm-auto-compare" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginTop: 14 }}><div style={{ padding: 12, borderRadius: 10, background: "white", border: "1px solid #e5e5eb" }}><strong style={{ fontSize: 12 }}>Automatic + fallback detection</strong><p style={{ margin: "6px 0 0", color: "#70707c", fontSize: 12 }}>The SDK gets a second adaptive pass when the first confidence gate cannot validate the Gemini candidate.</p></div><div style={{ padding: 12, borderRadius: 10, background: "white", border: "1px solid #e5e5eb" }}><strong style={{ fontSize: 12 }}>Visible mark only</strong><p style={{ margin: "6px 0 0", color: "#70707c", fontSize: 12 }}>The workflow targets the visible Gemini mark and does not remove or falsify SynthID or other invisible provenance.</p></div></div>
