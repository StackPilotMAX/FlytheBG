...
export async function POST(request: Request) {
  ...
  // Multipart framing adds a small amount of overhead. Reject obviously oversized requests
  // before FormData parsing when the client/proxy provides Content-Length.
  ...
  if (!actualMime || actualMime !== file.type) {
    return errorResponse("The file contents do not match a supported image format.", 415);
  }
  ...
  try {
    ...
    if (!response.ok) {
      ...
      const safe = response.status === 429
        ? "The service is busy. Try again shortly."
        : detail?.detail || "Image processing failed.";
    ...
    if (reason instanceof Error && reason.name === "AbortError") {
      return errorResponse("Image processing timed out. Try a smaller image.", 504);
    }
