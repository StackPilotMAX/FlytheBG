...
export const metadata: Metadata = {
  ...
  description: "Answers about FlytheBG browser AI, background removal, passport-photo tools, privacy, model licensing, downloads, and advertising.",
...
const generalFaqs = [
  ...
  ["Which AI model does FlytheBG use?", "FlytheBG uses IMG.LY's browser background-removal package with IS-Net model variants. Capable WebGPU devices can try the FP16 model, while constrained devices use the smaller quantized model. CPU/WASM is used as a fallback when WebGPU cannot finish."],
  ...
  ["Does using a passport-photo preset guarantee acceptance?", "No. FlytheBG helps with framing, physical dimensions, DPI, layout, and printing, but an issuing authority can also impose rules about expression, head position, lighting, clothing, photo age, editing, paper, and file requirements."],
  ...
  ["Why can the first background-removal run take longer?", "The browser may need to download the AI model and runtime assets before the first inference. Later runs may be faster when those assets are cached by the browser."],
