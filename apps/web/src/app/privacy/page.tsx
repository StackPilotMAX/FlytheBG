...
export default function PrivacyPage() {
  return <LegalPage title="Privacy & AI Policy" updated="21 August 2026">
    ...
    <p>Background removal currently integrates <code>@imgly/background-removal</code> version <strong>1.7.0</strong>. The production runtime can use IMG.LY&apos;s IS-Net model variants, including the smaller quantized path on constrained devices and an FP16 path on suitable higher-memory WebGPU devices, with CPU/WASM fallback when needed.</p>
    ...
    <p>FlytheBG may perform conservative local alpha-matte cleanup and, when memory permits, reapply a refined mask to higher-resolution source detail. This post-processing does not retrain IMG.LY&apos;s model weights and cannot recreate foreground detail the segmentation model never detected. FlytheBG does not use a visitor&apos;s selected image to train or fine-tune the IMG.LY model.</p>
