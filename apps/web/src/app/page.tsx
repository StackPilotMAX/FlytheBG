...
import { FlyTheBGCinematicHero } from "@/components/FlyTheBGCinematicHero";
...
export const metadata: Metadata = { title: { absolute: homeTitle }, description: homeDescription, keywords: ["FlyThe BG", "FlyTheBG", "background remover", "passport photo maker", "Gemini watermark remover", "browser image tools", "private image editor"], alternates: { canonical: "/" } };

const faqs = [
  ["Does FlyThe BG upload my photo?", "The core image workflows run in the browser. Your selected working media is processed in browser memory rather than being sent to a FlyThe BG image-processing server."],
  ["What can I do with FlyThe BG?", "Remove backgrounds, create measured passport-photo sheets, and use media cleanup tools for supported Gemini/Veo and other authorized media."],
  ["Is it free?", "Yes. FlyThe BG is designed as a free browser-first toolkit with no account required for the core workflows."],
] as const;
    ...
    <main className="flyHome">
      ...
      <section className="flySection flyTools" id="tools">
        ...
        <div className="flyToolGrid">
          ...
          <Link href="/features" className="flyToolCard"><span className="flyToolIndex">04</span><div><span className="flyToolLabel">ALL FEATURES</span><h3>See everything FlyThe BG can do.</h3><p>Browse the complete toolkit and choose the workflow you need.</p></div><b>↗</b></Link>
        </div>
      </section>
      <section className="flySection flyManifesto">
        <div className="flyManifestoMark">BG</div>
        <div><span className="flyEyebrow">02 / THE IDEA</span><h2>Your photo is the input.<br /><em>Not the product.</em></h2><p>FlyThe BG is built around a simple principle: use the browser for the work whenever the workflow allows it. Software, models and runtime assets may be downloaded, but your selected working image is designed to stay in browser memory during core processing.</p><Link className="flyInline" href="/privacy">Read the privacy & AI policy ↗</Link></div>
      </section>
      <section className="flySection flyProcess" id="workflow">
      ...
      <section className="flySection flyGuide">
        <div className="flyGuideCopy"><span className="flyEyebrow">04 / TRANSPARENCY</span><h2>Open about the<br /><em>technology.</em></h2><p>FlyThe BG uses third-party packages and models where they make the product better. Their attribution, licensing, limitations and browser behavior are documented instead of hidden.</p><div className="flyGuideLinks"><Link href="/model-disclosure">Model disclosure ↗</Link><Link href="/guides/browser-privacy">Browser privacy guide ↗</Link><Link href="/faq">Frequently asked questions ↗</Link></div></div>
      ...
      <section className="flySupport">
        <div><span className="flyEyebrow">06 / SUPPORT</span><h2>If FlyThe BG<br />saves you time, <em>buy it a book.</em></h2></div>
        <a href={SUPPORT_URL} target="_blank" rel="noopener noreferrer" className="flySupportButton">Support FlyThe BG <span>↗</span></a>
      </section>
      <div className="flyHomeAd"><AdPlaceholder slot="landing-inline-1" format="leaderboard" /></div>
      <footer className="flyHomeFooter"><FlytheBGLogo size={36} /><span>FlyThe BG — browser-first image & media tools.</span><Link href="/contact">Contact ↗</Link></footer>
    </main>
  );
}
