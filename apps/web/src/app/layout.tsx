...
const siteVideo = "https://d8j0ntlcm91z4.cloudfront.net/user_38xzZboKViGWJOttwIXH07lWA1P/hf_20260801_001207_ec20d138-aa45-4b2b-ab8c-bdc71607f240.mp4";
const defaultTitle = `FLYTHEBG | Free AI Background Remover & Passport Photo Maker`;
const defaultDescription = "FLYTHEBG is a free browser-first image toolkit for background removal, passport photos, and authorized AI-media utilities. Supported working media stays on the user's device.";
const googleVerification = process.env.NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION?.trim() || "";
const bingVerification = process.env.NEXT_PUBLIC_BING_SITE_VERIFICATION?.trim() || "";

export const metadata: Metadata = { metadataBase: new URL(appConfig.siteUrl), title: { default: defaultTitle, template: `%s | FLYTHEBG` }, description: defaultDescription, applicationName: "FLYTHEBG", keywords: ["FLYTHEBG", "FlytheBG", "free background remover", "Gemini watermark remover", "passport photo maker", "browser image tools"], authors: [{ name: "FLYTHEBG", url: appConfig.siteUrl }], creator: "FLYTHEBG", publisher: "FLYTHEBG", category: "Image editing tools", referrer: "origin-when-cross-origin", manifest: "/manifest.webmanifest", formatDetection: { telephone: false, address: false, email: false }, robots: { index: true, follow: true, nocache: false, googleBot: { index: true, follow: true, noimageindex: false, "max-video-preview": -1, "max-image-preview": "large", "max-snippet": -1 } }, openGraph: { title: defaultTitle, description: defaultDescription, siteName: "FLYTHEBG", locale: "en_US", type: "website", url: "/" }, twitter: { card: "summary", title: defaultTitle, description: defaultDescription }, appleWebApp: { capable: true, title: "FLYTHEBG", statusBarStyle: "black-translucent" }, verification: { ...(googleVerification ? { google: googleVerification } : {}), ...(bingVerification ? { other: { "msvalidate.01": bingVerification } } : {}) }, icons: { icon: [{ url: "/brand/flythebg-mark.svg", type: "image/svg+xml", sizes: "any" }], shortcut: "/brand/flythebg-mark.svg", apple: "/brand/flythebg-mark.svg" } };
...
export default function RootLayout({ children }: Readonly<{ children: React.ReactNode }>) { return <html lang="en"><head><meta name="monetag" content="5e777e0aa6ce027ca2e1a8ec1c8325b3" /><MonetizationHead /><Script src="https://www.googletagmanager.com/gtag/js?id=G-S50DRFD37X" strategy="lazyOnload" /><Script id="google-analytics" strategy="lazyOnload">{`window.dataLayer = window.dataLayer || []; function gtag(){window.dataLayer.push(arguments);} gtag('js', new Date()); gtag('config', 'G-S50DRFD37X');`}</Script></head><body className={`${instrumentSerif.variable} ${inter.variable}`}><div className="siteVideoBackdrop" aria-hidden="true"><video className="siteBackdropVideo" autoPlay muted loop playsInline preload="metadata"><source src={siteVideo} type="video/mp4" /></video><div className="siteBackdropWash" /></div><MotionLayer /><header className="siteHeader"><div className="shell navShell"><FlytheBGLogo priority /><nav className="navLinks" aria-label="Primary navigation"><Link href="/remove-background">Remove BG</Link><Link href="/ai-watermark-remover">Watermark Remover</Link><Link href="/features/passport-photo">Passport Photo</Link><Link href="/features">Features</Link><Link href="/guides">Guides</Link><Link href="/faq">FAQ</Link><Link href="/donate">Donate</Link></nav><div className="navUtility"><GitHubStars /><Link className="navCta" href="/donate">Support FlytheBG <span>↗</span></Link></div><Link className="mobileFeaturesButton" href="/features" aria-label="Open FlytheBG features">Features <span>↗</span></Link></div></header>{children}<footer className="siteFooter"><div className="shell footerGrid"><div className="footerBrand"><FlytheBGLogo size={42}/><p>Browser-first image and media tools. Supported workflows keep working media on your device.</p><div className="footerSocials"><a className="socialIconLink" href="https://github.com/StackPilotMAX" target="_blank" rel="noreferrer" aria-label="FlytheBG GitHub"><GithubIcon /><span>GitHub</span></a><a className="socialIconLink" href="https://www.instagram.com/flythebg/" target="_blank" rel="noreferrer" aria-label="FlytheBG Instagram"><InstagramIcon /><span>Instagram</span></a></div></div><div className="footerLinks"><GitHubStars /><Link href="/remove-background">Remove Background</Link><Link href="/ai-watermark-remover">Gemini Watermark Remover</Link><Link href="/features/passport-photo">Passport Photo</Link><Link href="/features">Features</Link><Link href="/guides">Guides</Link><Link href="/faq">FAQ</Link><Link href="/donate">Donate / Support</Link><Link href="/about">About</Link><Link href="/model-disclosure">Model Disclosure</Link><Link href="/privacy">Privacy & AI</Link><Link href="/terms">Terms</Link><Link href="/cookies">Cookies</Link><Link href="/contact">Contact</Link></div></div><div className="shell footerBottom"><span>© {new Date().getFullYear()} {appConfig.name}</span>{appConfig.contactEmail && <span>{appConfig.contactEmail}</span>}</div></footer><MonetizationScripts /></body></html>; }
