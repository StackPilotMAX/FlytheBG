...
export default function PassportPhotoPage() {
  return (
    <main className="passportPage">
      ...
      <section className="section passportNotes"><div className="shell infoCards"><article><span>Optional background removal</span><h2>Keep the original or remove the background.</h2><p>If you choose background removal, the same browser IMG.LY workflow used by FlytheBG&apos;s remover is applied before crop-frame positioning. If you keep the original background, that processing step is skipped.</p></article><article><span>Model disclosure</span><h2>rembg on Hugging Face.</h2><p>When background removal is selected, FlytheBG uses IMG.LY&apos;s browser package with IS-Net model variants. FlytheBG does not claim ownership of those model/runtime assets.</p><a className="textLink" href="/model-disclosure">Read Model & Open Source Disclosure ↗</a></article><article><span>Privacy</span><h2>Sheets are built in your browser.</h2><p>Crop-frame positioning, per-copy crop choices, physical-size conversion, sheet layout, PNG generation, and print preparation all happen in your browser. Only the background-removal step is routed through the authenticated server endpoint.</p></article></div></section>
